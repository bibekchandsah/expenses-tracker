    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <p className="text-lg font-bold text-gray-900 dark:text-white mt-1">{formatCurrency(stats.totalLent)}</p>
        </div>
        <div className="bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-200 dark:border-green-800 p-4">
          ...
          <p className="text-lg font-bold text-green-700 dark:text-green-400 mt-1">{formatCurrency(stats.totalReturned)}</p>
        </div>
                  ...
                  return (
                    <div key={lend.id}
                      ...
                      <div className="lg:col-span-1 text-right">
                        <span className="text-sm font-bold text-gray-900 dark:text-white">{formatCurrency(lend.amount)}</span>
                      </div>
                      ...
                      <div className="lg:col-span-1 text-right">
                        {+lend.returnedAmount > 0
                          ? <span className="text-sm font-semibold text-green-600 dark:text-green-400">{formatCurrency(lend.returnedAmount)}</span>
            ...
            {panelOpen && (
              <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">{formatCurrency(person.totalLent)}</span>
                      </div>
                      <div className="text-right">
                        {isSettled
                ...
                <div className="grid grid-cols-4 gap-1 px-4 py-3 bg-gray-50 dark:bg-gray-700/40 text-xs font-black text-gray-700 dark:text-gray-200 uppercase">
                  ...
                  <div className="text-right">{formatCurrency(stats.totalLent)}</div>
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">
                        {formatCurrency(person.totalReturned)} / {formatCurrency(person.totalLent)}
                      </span>
                    </div>
                    {/* Bar track */}
      ...
      <ConfirmDialog
        isOpen={!!deleteTarget}
        title="Delete Record"
        message={`Delete lend record for "${deleteTarget?.name}" (${formatCurrency(deleteTarget?.amount || 0)})? This cannot be undone.`}
