...
export default function Loan() {
  ...
  function handleExport() {
    ...
    exportCSV(headers, rows, 'loans.csv');
  }
  ...
  function handleResizeStart(e) {
    ...
    function handleMouseUp() {
      setIsResizing(false);
      try { localStorage.setItem('loanSidePanelWidth', String(sidePanelWidth)); } catch {}
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4">
          ...
          <CountUpValue value={formatCurrency(stats.totalBorrowed, currency)} className="text-lg font-bold text-gray-900 dark:text-white mt-1 block" />
        </div>
        <div className="bg-green-50 dark:bg-green-900/10 rounded-2xl border border-green-200 dark:border-green-800 p-4">
          ...
          <CountUpValue value={formatCurrency(stats.totalPaid, currency)} className="text-lg font-bold text-green-700 dark:text-green-400 mt-1 block" />
        </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/10 rounded-2xl border border-blue-200 dark:border-blue-800 p-4">
          ...
          <CountUpValue value={stats.lendersCount} className="text-lg font-bold text-blue-700 dark:text-blue-400 mt-1 block" />
        </div>
      </div>
                  ...
                  return (
                    <div key={loan.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors group">
                      ...
                      <div className="lg:hidden px-4 py-3">
                        ...
                        {(loan.reason || loan.description) && (
                          <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-0.5">
                            {loan.reason && <p className="text-xs text-gray-500 dark:text-gray-400"><span className="text-gray-400 dark:text-gray-500">Reason:</span> {capFirst(loan.reason)}</p>}
                            {loan.description && <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-1"><span className="text-gray-400 dark:text-gray-500">Note:</span> {capFirst(loan.description)}</p>}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="hidden lg:grid grid-cols-12 gap-1 px-5 py-3.5 items-center">
                        ...
                        <div className="col-span-2">
                          <span className="text-sm text-gray-600 dark:text-gray-400 truncate block" title={loan.reason}>{loan.reason ? capFirst(loan.reason) : '—'}</span>
                        </div>
                        <div className="col-span-1 text-right">
                          {+loan.paidAmount > 0
                        ...
                        <div className="col-span-2 pl-1">
                          {loan.description
                            ? <span className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1" title={capFirst(loan.description)}>{capFirst(loan.description)}</span>
                            : <span className="text-gray-300 dark:text-gray-600 text-sm">—</span>
                          }
                        </div>
